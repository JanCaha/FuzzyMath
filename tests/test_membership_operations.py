...
def test_fuzzy_and(quantize_precision):
    ...
    assert FuzzyAnd.Hamacher(fm_a, fm_b).membership.quantize(quantize_precision) == Decimal("0.265822784810127").quantize(quantize_precision)
...
def test_fuzzy_or(quantize_precision):
    ...
    assert FuzzyOr.Hamacher(fm_a, fm_b).membership.quantize(quantize_precision) == Decimal(0.826446280991736).quantize(quantize_precision)
...
def test_possibilistic_and(quantize_precision):
    ...
    assert PossibilisticAnd.Hamacher(pm_a, pm_b).possibility.quantize(quantize_precision) == Decimal("0.363636363636364").quantize(quantize_precision)
    assert PossibilisticAnd.Hamacher(pm_a, pm_b).necessity.quantize(quantize_precision) == Decimal("0.166666666666667").quantize(quantize_precision)
...
def test_possibilistic_or(quantize_precision):
    ...
    assert PossibilisticOr.Hamacher(pm_a, pm_b).possibility.quantize(quantize_precision) == Decimal("0.909090909090909").quantize(quantize_precision)
    assert PossibilisticOr.Hamacher(pm_a, pm_b).necessity.quantize(quantize_precision) == Decimal("0.636363636363636").quantize(quantize_precision)
